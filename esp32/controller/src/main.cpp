...
String sha256Hex(const String &input) { 
  uint8_t hash[32]; mbedtls_sha256_context ctx; 
  ...
  mbedtls_sha256_finish_ret(&ctx, hash); mbedtls_sha256_free(&ctx); const char* hex = "0123456789abcdef"; 
  char out[65]; 
...
bool syncOnce() { 
  ...
  HTTPClient http; http.begin(String(BASE_URL) + "/device/sync"); 
  http.addHeader("X-Device-Secret", DEVICE_SECRET); int code = http.GET(); 
  
  if (code != 200) {
    Serial.print("Sync HTTP "); Serial.println(code); http.end(); 
    return false; 
  } 
  
  StaticJsonDocument<8192> doc; 
  deserializeJson(doc, http.getString()); 
  http.end(); allowedPins.clear(); 
